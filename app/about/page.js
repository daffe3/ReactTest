...
export default async function About() { 
  ...
  const richTextOptions = {
    renderNode: {
      ...
      [BLOCKS.HEADING_2]: (node, children) => <h2 className="text-3xl font-bold mb-3 mt-6 text-text-dark border-b-2 border-primary-orange pb-2">{children}</h2>, 
      ...
      [BLOCKS.LIST_ITEM]: (node, children) => <li className="mb-1 break-words">{children}</li>, 
      [INLINES.HYPERLINK]: (node, children) => (
        <a href={node.data.uri} target="_blank" rel="noopener noreferrer" className="text-primary-orange hover:underline break-words"> 
          {children}
        </a>
      ),
    },
  ...
  return (
    <main className="flex-grow container mx-auto px-4 py-8">
      <h1 className="text-center mb-8 text-4xl font-bold text-text-dark">{aboutMe.title || 'About Me'}</h1> 
